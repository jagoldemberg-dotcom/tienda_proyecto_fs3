...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErrorDetails> manejarValidaciones(MethodArgumentNotValidException ex, WebRequest request) {
        ...
        ErrorDetails error = new ErrorDetails(LocalDateTime.now(), "Validación fallida: " + String.join(", ", errores), request.getDescription(false));
        return new ResponseEntity<>(error, HttpStatus.BAD_REQUEST);
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ErrorDetails> manejarGeneral(Exception ex, WebRequest request) {
        ErrorDetails error = new ErrorDetails(LocalDateTime.now(), "Ocurrió un error interno en el servidor.", request.getDescription(false));
