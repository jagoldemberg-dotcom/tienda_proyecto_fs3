...
class GlobalExceptionHandlerTest {
    ...
    void manejarRecursoNoEncontradoRetornaNotFound() {
        when(request.getDescription(false)).thenReturn("uri=/api/productos/1");
    ...
    @Test
    void manejarReglaDeNegocioRetornaBadRequest() {
        when(request.getDescription(false)).thenReturn("uri=/api/productos/1/comprar");

        ResponseEntity<ErrorDetails> response = handler.manejarReglaDeNegocio(
                new BusinessException("Stock insuficiente"), request);

        assertEquals(400, response.getStatusCode().value());
        assertTrue(response.getBody().getMessage().contains("Stock"));
    }
    ...
    void manejarValidacionesRetornaBadRequest() {
        ...
        when(request.getDescription(false)).thenReturn("uri=/api/productos");
    ...
    void excepcionesPersonalizadasConservanMensaje() {
        assertEquals("negocio", new BusinessException("negocio").getMessage());
